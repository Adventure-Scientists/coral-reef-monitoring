...
tryCatch({
  ...
  script_name <- "03-costa-rica-applicants.R"
  ...
  ######################################################################
  sheet <- "1vRA8hKCVepglGCBxlc_esJQiQKaXCgPATKdwVdbHBj8"
  
  applicants <- read.csv(paste0(data_folder, "all-applicants.csv"))
  
  costa_rica_applicants <- applicants %>% 
    filter(Dive_Location == "Costa Rica") %>% 
