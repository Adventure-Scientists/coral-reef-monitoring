...
tryCatch({
  ...
  script_name <- "04-colombia-applicants.R"
  ...
  ######################################################################
  sheet <- "1djKYg-HwCSDD8BtTgjAjVB_Ecjz4hsHyB0epB1We0cI"
  
  applicants <- read.csv(paste0(data_folder, "all-applicants.csv"))
  
  colombia_applicants <- applicants %>% 
    filter(Dive_Location == "Colombia") %>% 
